...
df1 = pd.read_csv('recording9.csv')  # Contains data for AF7
...
# Peak-Based Noise Filtering
def remove_peaks(data, threshold=5.0):
    """
    Identifies and removes peaks that exceed the given amplitude threshold.
    Peaks that exceed the threshold are replaced with the average of their neighbors.
    """
    ...
    # Replace peaks exceeding the threshold with the average of their neighbors
    for peak in peaks:
        if np.abs(data[peak]) > threshold:
            # Replace with average of neighbors (simple interpolation)
            if peak > 0 and peak < len(data) - 1:
                filtered_data[peak] = (data[peak - 1] + data[peak + 1]) / 2
            elif peak == 0:
                filtered_data[peak] = data[peak + 1]
            elif peak == len(data) - 1:
                filtered_data[peak] = data[peak - 1]
    
    return filtered_data

# Apply Peak-Based Noise Filtering
cleaned_signal_1 = remove_peaks(filtered_signal_1, threshold=5.0)
cleaned_signal_2 = remove_peaks(filtered_signal_2, threshold=5.0)
